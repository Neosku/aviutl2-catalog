...
pub fn init_settings(app: &AppHandle) -> std::io::Result<()> {
    ...
    // 本ソフトのconfigディレクトリを取得(%APPDATA%/Roaming/aviutl2-catalog)
    ...
    // settings.json のパスを決定
    let settings_path = catalog_config_dir.join("settings.json");
    // 読み込み(なくてもdefaultで初期化)
    let settings: Settings = Settings::load_from_file(&settings_path);
    ...
    // aviutl_dataのパスを決定(ポータブルモード ⇒ aviutl2_root/data not ⇒ ProgramData の aviutl2 フォルダ)
    ...
    // 実行中ファイルのディレクトリを取得（catalog.exe 等の親フォルダ）（フォールバックをエラー式にしてもいいかも）
    let catalog_exe_dir = std::env::current_exe().ok().and_then(|p| p.parent().map(PathBuf::from)).or_else(|| std::env::current_dir().ok()).unwrap_or_else(|| PathBuf::from("."));
    ...
    crate::log_info(&app, &format!("current_ver = {}", current_ver));

    // バージョンが変わったかどうかをチェック(初期起動時もここに移動)
    if settings.app_version != current_ver {
        crate::log_info(&app, &format!("バージョンが変わりました: {} -> {}", settings.app_version, current_ver));
        // crate::ensure_update_checker_plugin(app, &settings.aviutl2_root, &settings.plugin_dir)?;
        // ここで UpdateChecker.exe の移動を行う
        // 省略
    }
